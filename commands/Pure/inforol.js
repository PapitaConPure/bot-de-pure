...
module.exports = {
	...
	async execute(message, args) {
		if(args.length < 1)
			return message.reply({ content: `:x: ¡Debes ingresar al menos un parámetro!\nUsa \`${p_pure(message.guildId).raw}ayuda inforol\` para más información` });
		...
		if(!args.length)
			return message.reply({ content: ':warning: No se encontró ningún rol...' });
		...
		if(anaroles.length !== 0) {
			...
			message.reply({ embeds: [Embed[0]], allowedMentions: { parse: [] } }).then(sent => {
				sent.react(arrows[0])
