...
module.exports = {
	...
	execute(message, args) {
        ...
        if(global.jugadores[global.ndibujante] !== message.author.id) { //Cancelar comando si no fue ejecutado por el dibujante
            message.channel.send({ content: `:warning: Solo el dibujante, <@${global.jugadores[global.ndibujante]}> (jugador ${global.numeros[global.ndibujante]}), puede dar la recompensa.` });
            return;
        }
        if(!args.length) { //Cancelar si no se dio ningún parámetro
            message.channel.send({ content: `:warning: No mencionaste a ningún jugador, recuerda: \`${global.p_drmk}recomp <@usuario>\`.` });
            return;
        }
        if(!global.seleccionado || !global.dibujado) { //Cancelar si no ha empezado la adivinanza
            message.channel.send({ content: ':warning: Todavía no ha empezado la adivinanza.' });
            return;
        }
        ...
        if(idjugador === global.ndibujante) {
            message.channel.send({ content: ':warning: no puedes recompensarte solo.' });
            return;
        }
        ...
        if(idjugador !== -1) {
            ...
            } else message.channel.send({ content: ':warning: Ya se seleccionó un recompensado. Esperando aprobación de un moderador...' });
