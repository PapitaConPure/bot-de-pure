...
export function handleAndAuditError(
	...
	if (!brief) {
		if (Command.requestIsMessage(request))
			brief = 'Ha ocurrido un error al ejecutar un comando';
		...
		else if (request.isStringSelectMenu())
			brief = 'Ha ocurrido un error al procesar una acción de menú desplegable';
		else if (request.isModalSubmit())
			brief = 'Ha ocurrido un error al procesar una acción de ventana modal';
