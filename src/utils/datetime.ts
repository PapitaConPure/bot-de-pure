...
import { addDays, addHours, addMilliseconds, addMinutes, addSeconds, startOfDay } from 'date-fns';
 ...
 */
function utcStartOfTzDay<DateType extends Date, ResultDate extends Date = DateType>(
	date: DateArg<DateType>,
	options: StartOfDayOptions<ResultDate>,
) {
...
export const utcStartOfTzToday = (sanitizedTzCode: string = 'Etc/UTC') =>
	utcStartOfTzDay(Date.now(), { in: tz(sanitizedTzCode) });
/**@param sanitizedTzCode Código de zona horaria sanitizado (UTC por defecto)*/
export const utcStartOfTzYesterday = (sanitizedTzCode: string = 'Etc/UTC') =>
	utcStartOfTzDay(addDays(Date.now(), -1), { in: tz(sanitizedTzCode) });
/**@param sanitizedTzCode Código de zona horaria sanitizado (UTC por defecto)*/
export const utcStartOfTzTomorrow = (sanitizedTzCode: string = 'Etc/UTC') =>
	utcStartOfTzDay(addDays(Date.now(), +1), { in: tz(sanitizedTzCode) });

export const relativeDates = {
	beforeYesterday: {
		match: new Set(['anteayer', 'before yesterday', '一昨日']),
		getValue: (sanitizedTzCode = 'Etc/UTC') => addDays(utcStartOfTzToday(sanitizedTzCode), -2),
	},
	yesterday: {
		match: new Set(['ayer', 'yesterday', '昨日']),
		getValue: (sanitizedTzCode = 'Etc/UTC') => utcStartOfTzYesterday(sanitizedTzCode),
	},
	today: {
		match: new Set(['hoy', 'today', '今日']),
		getValue: (sanitizedTzCode = 'Etc/UTC') => utcStartOfTzToday(sanitizedTzCode),
	},
	tomorrow: {
		match: new Set(['mañana', 'tomorrow', '明日']),
		getValue: (sanitizedTzCode = 'Etc/UTC') => utcStartOfTzTomorrow(sanitizedTzCode),
	},
	afterTomorrow: {
		match: new Set(['pasado mañana', 'after tomorrow', '明後日']),
		getValue: (sanitizedTzCode = 'Etc/UTC') => addDays(utcStartOfTzToday(sanitizedTzCode), +2),
	},
} as const;
...
export function getDateComponentsFromString(str: string) {
	...
	if (seps.some((s) => str.startsWith(s)) || seps.some((s) => str.endsWith(s))) return;
 ...
 */
export function makeDateFromComponents(
	a: number,
	b: number,
	c: number,
...
export function parseDateFromNaturalLanguage(
	...
	if (dateComponents == null) return invalidDate();
...
export function parseTimeFromNaturalLanguage(str: string, utcOffset: number = 0): Date {
	...
	for (const relativeTime of Object.values(relativeTimes))
		if (relativeTime.match.has(str)) return addMinutes(relativeTime.getValue(), -utcOffset);
	...
	if (!Number.isNaN(+str)) {
		//Hora militar sencilla: HHmmss
		if (str.includes('-')) return invalidDate();

		if (str.length > 2 && str.length !== 4 && str.length !== 6) return invalidDate();

		const matches = str.match(/([0-9]{1,2})([0-9]{2})?([0-9]{2})?/);

		if (!matches) return invalidDate();

		rangesInclusive.h[1] = 23;
		timeComponents.h = +matches[1];
		timeComponents.m = +(matches[2] || 0);
		timeComponents.s = +(matches[3] || 0);
	} else if (/[午前後時分秒半]/.test(str)) {
		//Hora japonesa
		const gozen = '午前';
		const gogo = '午後';
		const han = '半';
		...
		if (usesGozen && usesGogo) return invalidDate();

		if (usesGozen && !(str.startsWith(gozen) || str.endsWith(gozen))) return invalidDate();

		if (usesGogo && !(str.startsWith(gogo) || str.endsWith(gogo))) return invalidDate();
		...
		if (usesHan && (mMatch || sMatch || !str.endsWith(han))) return invalidDate();

		if (!hMatch) return invalidDate();

		timeComponents.h = +hMatch[1];

		if (usesHan) timeComponents.m = 30;
		else {
		...
		if (str.includes(':')) {
			...
			const matches = str.match(
				/(am|pm)?([0-9]{1,2}):([0-9]{2})(?::([0-9]{2}))?(?:\.([0-9]{1,3}))?(am|pm)?/,
			);
		...
		if (meridiem1 && meridiem2) return invalidDate();
		...
		} else rangesInclusive.h[1] = 30; //Noche extendida
	}

	if (rangesInclusive.h[0] > timeComponents.h || timeComponents.h > rangesInclusive.h[1])
		return invalidDate();
	if (rangesInclusive.m[0] > timeComponents.m || timeComponents.m > rangesInclusive.m[1])
		return invalidDate();
	if (rangesInclusive.s[0] > timeComponents.s || timeComponents.s > rangesInclusive.s[1])
		return invalidDate();
	if (rangesInclusive.ms[0] > timeComponents.ms || timeComponents.ms > rangesInclusive.ms[1])
		return invalidDate();
