...
const logLevelPrefix = (logLevel: LogLevelKey) =>
	LogLevelColors[logLevel](LogLevelNames[logLevel].padEnd(longestLogLevelName));
...
export default function Logger(logLevel: LogLevelKey, prefix: string = '') {
	...
	function debug(...data: unknown[]) {
		LOG_LEVEL <= LogLevels.DEBUG
			&& console.log(logLevelPrefix('DEBUG'), prefix, logLevelOutput('DEBUG', ...data));
	}
	...
	function info(...data: unknown[]) {
		LOG_LEVEL <= LogLevels.INFO
			&& console.info(logLevelPrefix('INFO'), prefix, logLevelOutput('INFO', ...data));
	}
	 ...
	 * @see https://developer.mozilla.org/en-US/docs/Web/API/console/dir_static
	 */
	info.dir = (data: unknown, options?: Record<string, unknown>) => {
	...
	function warn(...data: unknown[]) {
		LOG_LEVEL <= LogLevels.WARN
			&& console.warn(logLevelPrefix('WARN'), prefix, logLevelOutput('WARN', ...data));
	}
