...
import type { AutocompleteInteraction, GuildBasedChannel, GuildTextBasedChannel } from 'discord.js';
import {
	Attachment,
...
import type {
	BaseParamType,
	BaseParamTypeMap,
	CommandArgumentGetFunction,
	CommandArguments,
	CommandRequest,
	ComplexCommandRequest,
	FeedbackOptions,
	FetchMessageFlagOptions,
	FlagCallback,
	GetMethodName,
	ParamPoly,
	ParamResult,
...
function fetchMessageFlagText(args: string[], i: number): string | undefined {
	...
	let lastIndex = i;
	while (lastIndex < args.length && !args[lastIndex].endsWith('"')) lastIndex++;
	const text = args
		.splice(i, lastIndex - i + 1)
		.join(' ')
		.slice(1);
...
function fetchMessageFlag(
	args: string[],
	flag: FetchMessageFlagOptions = {
		property: undefined as unknown as boolean,
		short: [],
		long: [],
		callback: undefined as unknown as FlagCallback<ParamResult>,
		fallback: undefined,
	},
) {
	...
	args.forEach((arg, i) => {
		...
		if (arg.startsWith('--')) {
			if (!flag.long?.length || !flag.long.includes(arg.slice(2))) return;
		...
		if (flag.short?.length && arg.startsWith('-') && arg !== '-') {
			...
			for (const c of flagChars) {
				...
				if (arg.length <= 2) return args.splice(i, 1);
	...
	if (flagValue == null) return flag.fallback;

	if (flag.callback == null) return flag.property ? flagValue : true;
...
const isBaseParamType = (pt: ParamType): pt is BaseParamType =>
	typeof pt === 'string' && paramTypes[pt] != null;

/**@description Devuelve si el parámetro es "de formato estricto".*/
const isParamTypeStrict = (pt: ParamType): pt is ParamTypeStrict =>
	typeof pt === 'object' && pt?.name != null && pt?.expression != null;
...
function groupQuoted(args: string[]): string[] {
	...
	for (const arg of args) {
		...
		} else if (isInsideQuotes) groupedTemp.push(arg);
		else result.push(arg);
	}

	if (isInsideQuotes) result.push(groupedTemp.join(' '));
...
export abstract class CommandOption {
	...
	setDesc(description: string) {
		if (!description)
			throw new InvalidCommandOptionAttributeError(
				'La descripción de la opción no puede ser null ni estar vacío',
			);
...
export class CommandParam extends CommandOption {
	...
	#poly: ParamPoly;
	#polymax: number;
	#autocompleteInner: AutocompleteFunction | undefined;
	...
	constructor(name: string, type: Exclude<ParamType | ParamType[], undefined | null>) {
		...
		if (!name)
			throw new InvalidCommandOptionAttributeError(
				'El nombre del parámetro no puede ser null ni estar vacío',
			);

		if (!type)
			throw new InvalidCommandOptionAttributeError(
				'El tipo del parámetro no puede ser null ni estar vacío',
			);
		...
		this.#poly = 'SINGLE';
		this.#polymax = 8;
		this.#autocompleteInner = undefined;
	}
	...
	autocomplete: AutocompleteFunction = async (interaction, query) => {
		if (!this.#autocompleteInner)
			throw new Error('autocomplete() endpoint reached without proper initialization.');
...
export class CommandFlag extends CommandOption {
	...
	get actualType() {
		if (!this._type)
			throw new Error(
				'Se esperaba que el tipo de esta opción estuviera inicializado para cuando se lo consultó',
			);
	...
	get identifier() {
		if (this._long?.length && Array.isArray(this._long)) return this._long[0];

		if (this._short?.[0]) return this._short[0];

		throw new Error(
			'No se definieron banderas cortas ni largas antes de acceder identificador de bandera',
		);
	}
}
...
export class CommandOptions {
	...
	constructor(request: CommandRequest | undefined = undefined) {
	...
	addParam(
		name: string,
		type: Exclude<ParamType | ParamType[], undefined | null>,
		desc: string,
		optionModifiers: { poly?: ParamPoly; polymax?: number; optional?: boolean } = {
			poly: undefined,
			polymax: undefined,
			optional: undefined,
		},
	...
	addFlag(
		...
		long: string | string[],
		desc: string,
		expression?: { name: string; type: Exclude<ParamType, undefined | null> },
	) {
		const commandFlag = expression
			? new CommandFlagExpressive(expression.name, expression.type)
			: new CommandFlag();
		commandFlag.setShort(short).setLong(long).setDesc(desc);
	...
	addOptions(...options: CommandOption[]) {
		for (const option of options) {
			...
			if (option.isCommandParam()) {
				...
				throw 'Comando inválido. Debes agregar una tag o una bandera.';
			}
	...
	static get fetchDependantTypes() {
		return CommandOptions.#fetchDependantTypes;
	}

	static get requestDependantTypes() {
		return CommandOptions.#requestDependantTypes;
	}
	...
	fetchMessageParamSync(args: string[], type: ParamType, whole: boolean = false): ParamResult {
		if (
			isBaseParamType(type)
			&& CommandOptions.requestDependantTypes.has(type)
			&& !this.#request
		)
			throw ReferenceError(
				'Se requiere un contexto para realizar este fetch. Usa <CommandOptions>.in(request)',
	...
	async fetchParam(
		...
		const param: CommandParam | undefined = this.params.get(slashIdentifier);

		if (!param)
			throw ReferenceError(
				`No se pudo encontrar un parámetro con el identificador: ${slashIdentifier}`,
			);

		if (param.poly !== 'SINGLE')
			throw TypeError(
				`No se puede devolver un solo valor con un poliparámetro: ${slashIdentifier}`,
			);

		if (Array.isArray(input)) {
			if (Array.isArray(param.type)) {
				const results = await Promise.all(
					param.type.map((pt) => this.fetchMessageParam(input, pt, whole)),
				);
				return results.find((r) => r);
			} else return this.fetchMessageParam(input, param.type, whole);
		}
		...
		if (Array.isArray(param.type)) {
			...
			for (const pt of param.type) {
				if (!isParamTypeStrict(pt)) getMethodName = paramTypes[pt].getMethod;
			...
			if (!isParamTypeStrict(param.type)) getMethodName = paramTypes[param.type].getMethod;
	...
	fetchParamPoly<TResult extends ParamResult, TFallback extends ParamResult | null | undefined>(
		input:
			...
			| Omit<CommandInteractionOptionResolver<'cached'>, 'getMessage' | 'getFocused'>,
		identifier: string,
		callbackFn: CommandArgumentGetFunction<TResult>,
		fallback: (() => TFallback) | TFallback | null | undefined = undefined,
	): (TResult | TFallback)[] {
		const option: CommandParam | undefined = this.params.get(identifier);

		if (!option)
			throw new ReferenceError(
				`No se pudo encontrar un Poly-parámetro con el identificador: ${identifier}`,
			);
		...
		if (!params.length)
			return [typeof fallback === 'function' ? fallback() : (fallback as TFallback)];
	 ...
	 */
	fetchFlag<T extends ParamResult = ParamResult, N extends ParamResult = ParamResult>(
		input:
			| CommandInteractionOptionResolver
			| Omit<CommandInteractionOptionResolver<'cached'>, 'getMessage' | 'getFocused'>
			| string[],
		identifier: string,
		output: FeedbackOptions<T, N> = { callback: undefined, fallback: undefined },
	): T | N | undefined {
		const flag = this.flags.get(identifier);

		if (!flag)
			throw new ReferenceError(
				`No se pudo encontrar una Flag con el identificador: ${identifier}`,
			);

		if (Array.isArray(input))
			return fetchMessageFlag(input, {
				...flag.structure,
				...(output as unknown as FetchMessageFlagOptions),
			}) as T | N;

		let getMethod: GetMethodName = 'getBoolean';

		if (flag.isExpressive()) {
			if (isParamTypeStrict(flag.actualType)) getMethod = 'getString';
			else getMethod = paramTypes[flag.actualType]?.getMethod ?? 'getString';
		}

		const flagValue = (input[getMethod] as (name: string) => T)(identifier);

		if (flagValue == null) return output.fallback;

		if (output.callback == null) return flagValue;

		return typeof output.callback === 'function'
			? output.callback(flagValue, true)
			: output.callback;
	}
...
const typeValidators: {
	[K in BaseParamType]: (v: unknown) => v is BaseParamTypeMap[K];
} = {
	NUMBER: (v): v is number => typeof v === 'number',
	TEXT: (v): v is string => typeof v === 'string',
	USER: (v): v is User => v instanceof User,
	MEMBER: (v): v is GuildMember => v instanceof GuildMember,
	GUILD: (v): v is Guild => v instanceof Guild,
	CHANNEL: (v): v is GuildBasedChannel => v instanceof GuildChannel,
	MESSAGE: (v): v is Message<boolean> => v instanceof Message,
	ROLE: (v): v is Role => v instanceof Role,
	EMOTE: (v): v is string => typeof v === 'string',
	IMAGE: (v): v is string | Attachment => typeof v === 'string' || v instanceof Attachment,
	FILE: (v): v is string | Attachment => typeof v === 'string' || v instanceof Attachment,
	URL: (v): v is URL => v instanceof URL,
...
export class CommandOptionSolver<TArgs extends CommandArguments = CommandArguments> {
	...
	get remainder() {
		if (this.isMessageSolver(this.#args)) return this.#args.join(' ');
	...
	getStringUnchecked(
		identifier: string,
		getRestOfMessageWords: boolean = false,
	): string | undefined {
	 ...
	 */
	getString(identifier: string, getRestOfMessageWords: boolean = false) {
		if (this.isInteractionSolver(this.#args))
			return this.#args.getString(identifier) ?? undefined;

		const result = this.#getResultFromParamSync(identifier, getRestOfMessageWords);
		return CommandOptionSolver.asStringOrUndefined(result);
	}
	...
	getUser(identifier: string, getRestOfMessageWords: boolean = false): User | undefined {
		if (this.isInteractionSolver(this.#args))
			return this.#args.getUser(identifier) ?? undefined;
	...
	async getGuild(
		...
		if (this.isInteractionSolver(this.#args))
			return fetchGuild(this.#args.getString(identifier) as string);
	...
	getAttachment(identifier: string): Attachment | undefined {
		if (this.isInteractionSolver(this.#args)) {
			...
			if (!attachment || !CommandOptionSolver.isAttachment(attachment)) return;
	...
	async getMessage(
		...
		if (this.isInteractionSolver(this.#args)) {
			const message = await fetchMessage(
				...
				this.#request as { channel: GuildTextBasedChannel; guild: Guild },
			);
	...
	#getDateComponentsFromMessage() {
		...
		if (dateComponents.some((d) => Number.isNaN(+d))) return;

		if (seps.some((s) => rawDateStr.startsWith(s))) return;

		if (firstArg.startsWith('"')) {
			if (seps.some((s) => rawDateStr.endsWith(s))) return;
		} else {
			if (seps.some((s) => rawDateStr.endsWith(s)) && dateComponents.length >= 3) return;
			...
			while (!done && this.#args.length && dateComponents.length < 3) {
				...
				if (seps.includes(arg)) {
					...
					if (Number.isNaN(+this.#args[0])) return;
					...
					if (!subArgs.length || dateComponents.length + subArgs.length > 3) return;
	...
	#getDateComponentsFromInteraction(identifier: string) {
		...
		return dateStr ? getDateComponentsFromString(dateStr) : undefined;
	}
	...
	#getRelativeDateCompatibleMessageArgs() {
		...
		return this.#args
			.slice(0, 2)
			.filter((a) => relativeDateKeys.has(a))
			.join(' ');
	}
	...
	#getTimeStringFromMessage() {
		...
		const directAppendHandler: __TimeStrMatchHandler = (x) => {
			timeStr += x;
		};
		...
		while (this.hasNext()) {
			...
			for (const [id, availableMatch] of Object.entries(availableMatches)) {
				...
				availableMatch.subtr?.forEach((id) => {
					const m = availableMatches[id];
					if (m) m.limit--;
					if (m.limit <= 0) delete availableMatches[id];
				});

				availableMatch.limit--;
				if (availableMatch.limit <= 0) delete availableMatches[id];
	...
	async parsePolyParam<TFallback extends ParamResult | null>(
		...
		if (this.isMessageSolver(this.#args)) {
			if (
				...
				&& !isParamTypeStrict(option.type)
				&& paramTypes[option.type]?.getMethod === 'getAttachment'
			) {
				this.ensureRequistified();
			...
			const arrArgs = this.#regroupMessageArgs(this.#args, regroupMethod, {
				mentionableType,
				messageSep,
			});
			if (!arrArgs.length)
				return this.#makePolyParamFallbackResult(fallback) as (ParamResult | TFallback)[];
			...
			// biome-ignore lint/complexity/noCommaOperator: Atajo
			while (arrArgs.length && ((arrArg = arrArgs[i]), i++) < polymax) {
				...
				if (Array.isArray(option.type)) {
					const results = await Promise.all(
						option.type.map((pt) =>
							this.#options.fetchMessageParam(arrArgs, pt, false),
						),
					);
					result = results.find((r) => r);
				} else result = await this.#options.fetchMessageParam(arrArgs, option.type, false);

				if (result != null) results.push(result);
				else failedPayload?.push(arrArg);
			}
		...
		const method =
			Array.isArray(option.type) || isParamTypeStrict(option.type)
				? this.#args.getString
	 ...
	 * @param identifier El identificador del poli-parámetro que representa a cada {@linkcode CommandParam} asociado
	 */
	parsePolyParamSync<TFallback extends ParamResult = undefined>(
		identifier: string,
		parseOptions: PolyParamParsingOptions<TFallback> = {},
	): (ParamResult | TFallback)[] {
		const {
			regroupMethod = 'SEPARATOR',
			messageSep = ',',
			fallback = undefined,
		...
		if (this.isMessageSolver(this.#args)) {
			...
			if (!arrArgs.length) return this.#makePolyParamFallbackResult(fallback);
			...
			while (arrArgs.length && ((arrArg = arrArgs[i]), i++) < polymax) {
				...
				if (Array.isArray(option.type)) {
					const results = option.type.map((pt) =>
						this.#options.fetchMessageParamSync(arrArgs, pt, false),
					);
					result = results.find((r) => r);
				} else result = this.#options.fetchMessageParamSync(arrArgs, option.type, false);
		...
		const method =
			Array.isArray(option.type) || isParamTypeStrict(option.type)
				...
				: (this.#args[
						paramTypes[option.type].getMethod as GetMethodName
					] as CommandArgumentGetFunction);
	...
	#makePolyParamFallbackResult<TFallback = undefined>(fallback: TFallback): Array<TFallback> {
		return fallback != null ? [typeof fallback === 'function' ? fallback() : fallback] : [];
	}

	#regroupMessageArgs(
		args: string[],
		regroupMethod: RegroupMethod,
		options: { mentionableType?: ParamType | Array<ParamType>; messageSep?: string } = {},
	) {
		const { mentionableType: type = 'USER', messageSep: sep = ',' } = options;

		switch (regroupMethod) {
			// biome-ignore lint/suspicious/noFallthroughSwitchClause: Es de esas cosas que si funcionan no las tocás
	...
	hasFlag(identifier: string) {
		if (this.isInteractionSolver(this.#args)) return this.#args.getBoolean(identifier) ?? false;
	 ...
	 */
	flagIf<TResult extends ParamResult, TFallback extends ParamResult = undefined>(
		identifier: string,
		positiveResult: TResult,
		negativeResult?: TFallback,
	): TResult | TFallback | undefined {
		return this.#options.fetchFlag(this.#args, identifier, {
	...
	flagExprIf<TResult extends ParamResult, TFallback extends ParamResult = undefined>(
		...
		callback?: FlagCallback<TResult>,
		fallback?: TFallback,
	): ReturnType<FlagCallback<TResult>> | TFallback | undefined {
		callback ??= (<TResult>(x: TResult) => x as TResult) as FlagCallback<TResult>;
	...
	as<TParam extends BaseParamType>(
		type: TParam,
		value: ParamResult,
	...
	asRequired<TParam extends BaseParamType>(
		type: TParam,
		value: ParamResult,
	): BaseParamTypeMap[TParam] {
		const finalValue = this.as(type, value);

		if (finalValue == null) {
			throw new UnexpectedValueError('Malformed parameter', {
				expected: type,
				received: 'undefined',
			});
		}
	...
	//#region Casters
	static asBooleanOrUndefined(paramResult: ParamResult) {
		return assertResultAsTypeOrUndefined<boolean>(paramResult, 'boolean');
	}

	static asBoolean(paramResult: ParamResult) {
		return assertResultAsType<boolean>(paramResult, 'boolean');
	}

	static asBooleansOrUndefined(paramResults: ParamResult[]) {
		return paramResults.map((r) => CommandOptionSolver.asBooleanOrUndefined(r));
	}
	...
	static asStringOrUndefined(paramResult: ParamResult) {
		return assertResultAsTypeOrUndefined<string>(paramResult, 'string');
	}

	static asString(paramResult: ParamResult) {
		return assertResultAsType<string>(paramResult, 'string');
	}

	static asStringsOrUndefined(paramResults: ParamResult[]) {
		return paramResults.map((r) => CommandOptionSolver.asStringOrUndefined(r));
	}
	...
	static asNumberOrUndefined(paramResult: ParamResult) {
		return assertResultAsTypeOrUndefined<number>(paramResult, 'number');
	}

	static asNumber(paramResult: ParamResult) {
		return assertResultAsType<number>(paramResult, 'number');
	}

	static asNumbersOrUndefined(paramResults: ParamResult[]) {
		return paramResults.map((r) => CommandOptionSolver.asNumberOrUndefined(r));
	}
	...
	static isUser(paramResult: ParamResult): paramResult is User {
		if (paramResult == null) return false;
	...
	#expectParam(identifier: string) {
		...
		if (!option) throw `No se encontró una opción bajo el identificador: ${identifier}`;

		if (!option.isCommandParam()) throw 'Se esperaba un identificador de parámetro de comando';

		return option;
	}

	#getResultFromParamSync(
		identifier: string,
		getRestOfMessageWords: boolean = false,
	): ParamResult {
		if (!this.isMessageSolver(this.#args))
		...
		if (Array.isArray(option.type)) {
			const results = option.type.map((pt) =>
				this.#options.fetchMessageParamSync(arrArgs, pt, getRestOfMessageWords),
			);
			finalResult = results.find((r) => r);
		} else
			finalResult = this.#options.fetchMessageParamSync(
				arrArgs,
				option.type,
				getRestOfMessageWords,
			);

		return finalResult;
	}

	async #getResultFromParam(
		identifier: string,
		getRestOfMessageWords: boolean = false,
	): Promise<ParamResult> {
		if (!this.isMessageSolver(this.#args))
		...
		if (Array.isArray(option.type)) {
			const results = await Promise.all(
				option.type.map((pt) =>
					this.#options.fetchMessageParam(arrArgs, pt, getRestOfMessageWords),
				),
			);
			return results.find((r) => r);
		} else return this.#options.fetchMessageParam(arrArgs, option.type, getRestOfMessageWords);
	}
...
export class InvalidCommandOptionAttributeError extends Error {
	constructor(message?: string) {
		super(message);
		this.name = InvalidCommandOptionAttributeError.name;
	}
}
...
export class MalformedParameterError extends Error {
	constructor(message?: string) {
		super(message);
		this.name = MalformedParameterError.name;
	}
}

function assertResult(
	value: ParamResult,
	expectedType: SimpleTypeExpr,
	predicate?: (value: ParamResult) => boolean,
) {
...
function assertResultAsTypeOrUndefined<T>(
	value: ParamResult,
	expectedType: SimpleTypeExpr,
	predicate?: (value: ParamResult) => boolean,
): T | undefined {
	...
	if (!assertResult(value, expectedType, predicate)) {
		throw new UnexpectedValueError('Malformed parameter', {
...
function assertResultAsType<T>(
	value: ParamResult,
	expectedType: SimpleTypeExpr,
	predicate?: (value: ParamResult) => boolean,
): T {
	const finalValue = assertResultAsTypeOrUndefined<T>(value, expectedType, predicate);
