...
import { navigationRows, paginateRaw, rand } from '@/func';
...
async function loadPageNumber(request: Exclude<AnyRequest, AutocompleteInteraction>, page: number) {
	...
	const emotePages = paginateRaw(emotes, 10).map((page) =>
		page.map((item) => `\`${item.name.padEnd(24)}\`${item}`).join('\n'),
	);
...
const options = new CommandOptions()
	.addParam(
		'perrito',
		'TEXT',
		'para especificar un perrito a enviar (por nombres identificadores)',
		{ optional: true },
	)
	.addFlag(
		'ltaeh',
		['lista', 'todo', 'todos', 'ayuda', 'everything', 'all', 'help'],
		'para mostrar una lista de todos los perritos',
	)
...
const command = new Command('perrito', tags)
	...
	.setLongDescription(
		'Comando cachorro de Taton. Puedes ingresar una palabra identificadora para enviar un perrito en específico o ver una lista de perritos. Si no ingresas nada, se enviará un perrito aleatorio',
	)
	.setOptions(options)
	.setExecution(async (request, args) => {
		...
		if (args.hasFlag('lista')) return loadPageNumber(request, 0);

		const emotes = getEmotesList(request);
		let perrito = args.getString('perrito');

		if (!perrito) return request.reply({ content: `${emotes[rand(emotes.length)]}` });

		perrito = perrito.normalize('NFD').replace(/([aeiou])\u0301/gi, '$1');
		const perritoEmoji = emotes.find((emote) =>
			emote.name.toLowerCase().startsWith(perrito.toLowerCase()),
		);
