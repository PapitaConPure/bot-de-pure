...
const options = new CommandOptions()
	.addParam(
		'cantidad',
		'NUMBER',
		'para especificar la cantidad de mensajes a borrar (sin contar el mensaje del comando)',
		{ optional: true },
	)
	.addFlag('um', ['usuario', 'miembro'], 'para especificar de qué usuario borrar mensajes', {
		name: 'user',
		type: 'USER',
	});
...
const command = new Command(
	...
	.setExecution(async (request, args) => {
		...
		if (request.isInteraction) await request.deferReply({ flags: MessageFlags.Ephemeral });

		if (request.isMessage) await safeDelete(request);

		const cappedAmount = Math.max(2, Math.min(amount, 100));

		await bulkDeleteMessages(request.channel, cappedAmount, user);

		if (request.isInteraction) return request.editReply({ content: '✅ Mensajes eliminados' });
	});
