...
const options = new CommandOptions().addParam(
	'usuario',
	'USER',
	'para especificar el usuario a bannear',
);
...
const command = new Command('papa-ban', tags)
	...
	.setExecution(async (request, args) => {
		...
		if (!user || user.id === papita) return request.reply({ content: '⚠️ Usuario inválido' });

		const userConfig =
			(await UserConfig.findOne({ userId: user.id })) || new UserConfig({ userId: user.id });
		userConfig.banned = !userConfig.banned;
		await userConfig.save().then(() => recacheUser(user.id));

		return request.reply({
			content: userConfig.banned ? '✅ Usuario banneado' : '✅ Usuario desbanneado',
		});
	});
