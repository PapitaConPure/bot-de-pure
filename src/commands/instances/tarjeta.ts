...
const options = new CommandOptions().addOptions(
	...
	new CommandParam('puntaje', 'NUMBER').setDesc('para establecer el puntaje'),
	new CommandParam('fecha', 'DATE').setDesc('para establecer la fecha'),
	new CommandFlag()
		.setShort('b')
		.setLong(['nobomb', 'nb'])
...
const command = new Command('tarjeta', tags)
	...
	.setExecution(async (request, args) => {
		...
		if (!bg)
			return request.reply({
				content:
					'⚠️ Debes ingresar un nombre o número de juego válido. Solo se permiten juegos oficiales de danmaku tradicional',
			});
		...
		if (dateNumbers.some((n) => Number.isNaN(+n)))
			return request.reply({
				content: '⚠️ Fecha inválida. Asegúrate de seguir el formato DD/MM/AAAA',
			});
		...
		try {
			...
			challengeImages.forEach((challengeImage) =>
				ctx.drawImage(challengeImage, 0, 0, canvas.width, canvas.height),
			);
			...
			const replyContent = {
				...
				files: [
					new AttachmentBuilder(canvas.toBuffer('image/webp'), { name: 'tarjeta.webp' }),
				],
			};
