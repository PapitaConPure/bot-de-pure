...
const command = new Command('colgar', tags)
	.setAliases('castrar', 'colgate', 'hang', 'hanged')
	...
	.setExecution(async (request, args) => {
		...
		if (everyone) {
			const embed = new EmbedBuilder().setTitle('Colgar a todos').addFields({
				name: 'Confirmar operación',
				value: '¿Quieres colgar o descolgar a todos?',
			});
		...
		if (!member)
			return request.reply({
				content:
					'⚠️ Debes indicar un miembro a colgar (y no, "ESTAAAAAA" no es un miembro válido)',
				ephemeral: true,
			});
		...
		let err: string;
		if (wasHanged)
			await member.roles.add(hangedDollId, `Colgado por ${user.tag}`).catch(() => {
				err = 'Oe oe espérate conchetumare eri muy poderoso wtf';
			});
		else
			await member.roles
				.remove(member.roles.cache.find((r) => r.id === hangedDollId))
				.catch(() => {
					err = 'wtf dios mío qué está pasando';
				});
	...
	.setButtonResponse(async function addHanged(interaction, userId) {
		...
		return Promise.all(
			interaction.guild.members.cache
				...
				.map((member) =>
					member.roles.add(hangedDollId, `Colgado por ${interaction.user.tag}`),
				),
		);
	})
	.setButtonResponse(async function removeHanged(interaction, userId) {
		if (interaction.user.id !== userId)
			return interaction.reply({ content: '❌ No permitido', ephemeral: true });
		const embed = new EmbedBuilder().setTitle('Colgada en Masa deshecha').addFields({
			name: 'Se descolgó a todos los miembros',
			value: 'Si siguen vivos, bien por ellos~',
		});
		await interaction.update({
			embeds: [embed],
			components: [],
	...
	.setButtonResponse(async function cancelHanged(interaction, userId) {
		if (interaction.user.id !== userId)
			return interaction.reply({ content: '❌ No permitido', ephemeral: true });
		const embed = new EmbedBuilder().setTitle('Colgada en Masa cancelada').addFields({
			name: 'Se canceló la operación',
			value: 'Supongo que van a vivir (o no) un día más',
		});
