...
const command = new Command('repetir', tags)
	...
	.setSelectMenuResponse(async function setLoopMode(interaction) {
		...
		try {
			...
			const queue =
				player.queues.get(interaction.guildId)
				?? (await tryRecoverSavedTracksQueue(interaction));
			if (!queue?.currentTrack) {
				const embed = makePuréMusicEmbed(interaction).setTitle(
					translator.getText('queueLoopTitleNoTrack'),
				);
				return interaction.reply({
					embeds: [embed],
					ephemeral: true,
			...
			switch (loopMode) {
				...
				case 'track': {
					...
					embed = makePuréMusicEmbed(interaction, service.color, service.iconUrl, [
						queueInfo,
					])
			...
			console.error(err);
			const embed = makePuréMusicEmbed(interaction, Colors.Red).setTitle(
				translator.getText('somethingWentWrong'),
			);
