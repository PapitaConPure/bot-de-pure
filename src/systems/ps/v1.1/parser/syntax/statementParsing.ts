...
export function parseBlockBody(parser: Parser, ...closeTokenKinds: TokenKind[]): BlockBody {
	if (!closeTokenKinds.length) closeTokenKinds.push(TokenKinds.BLOCK_CLOSE);
	...
	while (parser.hasTokens && !parser.current.isAny(...closeTokenKinds)) {
		if (!parser.current.isStatement)
			throw parser.TuberParserError(
				`Se intentó iniciar una nueva sentencia, pero en lugar de un indicador de sentencia se recibió: *${translateTokenKind(parser.current.kind)}*`,
			);
	...
	if (earlyAbortIndex !== -1) return body.slice(0, earlyAbortIndex + 1);
...
export function parseBlockStatement(parser: Parser): BlockStatement {
	...
	const closeToken = parser.expect(
		TokenKinds.BLOCK_CLOSE,
		`Se esperaba un cierre de bloque con Sentencia \`FIN\` en algún punto después del indicador \`BLOQUE\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
	);
...
export function parseConditionalStatement(parser: Parser): ConditionalStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión luego de indicador \`SI\`, pero la Sentencia finalizó con: *${parser.current.translated}*`,
	);
	const test = parser.parseExpression(BindingPowers.ASSIGNMENT);

	const consequentBlock = parseBlockBody(
		parser,
		TokenKinds.BLOCK_CLOSE,
		TokenKinds.ELSE,
		TokenKinds.ELSE_IF,
	...
	parser
		.ensureAny(TokenKinds.BLOCK_CLOSE, TokenKinds.ELSE, TokenKinds.ELSE_IF)
		.orFail(
			`Se esperaba un cierre de bloque en algún punto después del indicador \`SI\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
		);
	...
	if (parser.current.is(TokenKinds.ELSE)) {
		...
		const closeToken = parser.expect(
			TokenKinds.BLOCK_CLOSE,
			`Se esperaba un cierre de bloque con Sentencia \`FIN\` en algún punto después del indicador \`SINO\` en la línea en la línea **${alternationToken.line}**, columnas **${alternationToken.start}** a **${alternationToken.end}**. Sin embargo, no se encontró ninguna`,
		);
...
export function parseWhileLoopStatement(parser: Parser): WhileStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión luego de indicador \`MIENTRAS\`, pero la Sentencia finalizó con: *${parser.current.translated}*`,
	);
	const test = parser.parseExpression(BindingPowers.ASSIGNMENT);
...
export function parseDoWhileLoopStatement(parser: Parser): DoUntilStatement {
	...
	const closeToken = parser.expect(
		TokenKinds.UNTIL,
		`Se esperaba un cierre de bloque con Sentencia \`HASTA\` en algún punto después del indicador \`HACER\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
	);
	const body = makeBlockStmt(doWhileBlock, openToken, closeToken);

	parser.ensureExpression(
		`Se esperaba una expresión luego de indicador \`HASTA\`, pero la Sentencia \`HACER\` asociada finalizó con: *${parser.current.translated}*`,
	);
	const test = parser.parseExpression(BindingPowers.ASSIGNMENT);
...
export function parseRepeatLoopStatement(parser: Parser): RepeatStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión luego de indicador \`REPETIR\`, pero la Sentencia finalizó con: *${parser.current.translated}*`,
	);
	...
	const closeToken = parser.expect(
		TokenKinds.BLOCK_CLOSE,
		`Se esperaba un cierre de bloque con Sentencia \`FIN\` en algún punto después del indicador \`REPETIR\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
	);
...
export function parseForEachLoopStatement(parser: Parser): ForEachStatement {
	const openToken = parser.advance();
	const identifier = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un identificador en Sentencia \`PARA CADA\`, pero se recibió: *${parser.current.value}*`,
	);
	parser.expect(TokenKinds.IN);
	parser.ensureExpression(
		`Se esperaba una expresión de contenedor luego de operador \`en\` en Sentencia \`PARA CADA\`, pero la misma finalizó con: *${parser.current.translated}*`,
	);
...
export function parseForLoopStatement(parser: Parser): ForStatement {
	...
	const identifier = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un literal de identificador en Sentencia \`PARA\`, pero se recibió: *${parser.current.value}*`,
	).value as string;
	let full = false;
	...
	if (parser.current.is(TokenKinds.FROM)) {
		const openToken = parser.advance();
		parser.ensureExpression(
			`Se esperaba una expresión luego de operador \`desde\` en Sentencia \`PARA\` corta, pero la misma finalizó con: *${parser.current.translated}*`,
		);
		const from = parser.parseExpression(BindingPowers.COMMA);
		parser.expect(
			TokenKinds.UNTIL,
			`Se esperaba operador \`hasta\` en Sentencia \`PARA\` corta, pero se recibió: *${parser.current.value}*`,
		);
		parser.ensureExpression(
			`Se esperaba una expresión luego de operador \`hasta\` en Sentencia \`PARA\` corta, pero la misma finalizó con: *${parser.current.translated}*`,
		);
		...
		const closeToken = parser.expect(
			TokenKinds.BLOCK_CLOSE,
			`Se esperaba un cierre de bloque con Sentencia \`FIN\` en algún punto después del indicador \`PARA\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
		);
	...
	full = true;
	parser.expect(
		TokenKinds.ASSIGNMENT,
		`Se esperaba el operador \`con\` o \`desde\` luego de identificador en Sentencia \`PARA\`, pero se recibió: *${parser.current.value}*`,
	);
	parser.ensureExpression(
		`Se esperaba una expresión luego de operador \`con\` en paso de inicialización de Sentencia \`PARA\` larga, pero la misma finalizó con: *${parser.current.translated}*`,
	);
	const init = parser.parseExpression(BindingPowers.COMMA);
	parser.expect(
		TokenKinds.WHILE,
		`Se esperaba \`MIENTRAS\` en sentencia \`PARA\` larga, pero se recibió: *${parser.current.value}*`,
	);
	parser.ensureExpression(
		`Se esperaba una expresión en paso de condición de Sentencia \`PARA\` larga, pero la misma finalizó con: *${parser.current.translated}*`,
	);
	const test = parser.parseExpression(BindingPowers.ASSIGNMENT);
	parser.ensureStatement(
		`Se esperaba una Sentencia en paso de actualización de Sentencia \`PARA\` larga, pero la misma finalizó con: *${parser.current.translated}*`,
	);
	...
	const closeToken = parser.expect(
		TokenKinds.BLOCK_CLOSE,
		`Se esperaba un cierre de bloque con Sentencia \`FIN\` en algún punto después de Sentencia \`PARA\` en la línea **${openToken.line}**, columnas **${openToken.start}** a **${openToken.end}**. Sin embargo, no se encontró ninguna`,
	);
...
export function parseExpressionStatement(parser: Parser): ExpressionStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión a ejecutar inmediatamente después de indicador \`${startToken.value}\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
...
export function parseReadStatement(parser: Parser): ReadStatement {
	...
	const dataKind = parser
		...
		.orFail(
			`Se esperaba un tipo de Entrada antes de expresión receptora en Sentencia \`LEER\`, pero se recibió: *${parser.current.translated}*`,
		);

	let optional = false;
	parser.ensureExpression(
		`Se esperaba una expresión receptora en Sentencia \`LEER\`, pero se recibió: *${parser.current.translated}*`,
	);
...
export function parseReadStmtModifiers(
	parser: Parser,
	dataKind: Token,
): { preModifiers: ReadStatementPreModifier[]; modifiers: ReadStatementModifier[] } {
	const preModifiers = [];
	const modifiers = [];
...
export function parseReadStmtModifier(
	preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier[],
	parser: Parser,
	dataKind: Token,
	...
	if (!modifier)
		throw parser.TuberParserError(
			`Se intentó designar una directiva de Entrada de Usuario con \`${parser.current.value}\`, pero no existe ninguna para ${dataKind.translated}`,
		);
...
function percentOrFactorFromString(str: string): { percent?: number; factor?: number } {
	...
	if (pcnt) return { percent: +pcnt[1] };

	const fac = numStr.match(/^(?!_)(([0-9_]+([.][0-9]*)?)|([.][0-9]+))\s*$/);
	if (fac) return { factor: +fac[1] };

	throw TuberInputError('Se esperaba un factor o porcentaje en Entrada numérica');
}

function parseReadStmtNumberModifier(
	preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier[],
	parser: Parser,
) {
	const opToken = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un operador de formato para la definición de Entrada de Usuario, pero se recibió: *${parser.current.translated}*`,
	);
	...
	switch (name) {
		case 'como': {
			...
			const wordMappings = {
				...
				porcentaje: () =>
					preModifiers.push((v) => {
						...
						if (factor) return `${factor * 100}`;
					}),
				factor: () =>
					preModifiers.push((v) => {
						const { percent, factor } = percentOrFactorFromString(v);

						if (percent) return `${percent / 100}`;
				...
				opcion: () => {
					...
					const sequence = require('./expressionParsing').parseSequenceExpression(
						parser,
						first,
						BindingPowers.COMMA,
					...
					modifiers.push((v: NumberValue, it, scope) => {
						...
						if (i < 0 || i >= sequence.expressions.length) return v;
			...
			if (!setModifier)
				throw parser.TuberParserError(
					`Palabra inválida en directiva de Entrada de Usuario de Número "cómo": \`${identifier.value}\``,
				);
		...
		case 'entre': {
			...
			modifiers.push((v: NumberValue, it, scope) => {
				...
				if (v.value < leftVal.value) return makeNumber(leftVal.value);

				if (v.value > rightVal.value) return makeNumber(rightVal.value);
		...
		default:
			throw parser.TuberParserError(
				`La palabra \`${name}\` no es válida para una Entrada de Tipo Número`,
				opToken,
			);
	}
}

function parseReadStmtTextModifier(
	_preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier[],
	parser: Parser,
) {
	if (parser.current.is(TokenKinds.IDENTIFIER)) {
		...
		switch (name) {
			case 'normalizada':
			case 'normalizado':
				modifiers.push((/**@type {TextValue}*/ v: TextValue) =>
					makeText(toLowerCaseNormalized(v.value)),
				);
				break;

			default:
				throw parser.TuberParserError(
					`La palabra \`${name}\` no es válida para una Entrada de Tipo Texto`,
					token,
				);
		}
	...
	parser.expect(
		TokenKinds.IN,
		`Se esperaba una palabra de formato para la definición de Entrada de Usuario, pero se recibió: *${parser.current.translated}*`,
	);
	const caseToken = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba: \`mayusculas\`, \`minusculas\` o similares en definición de formato de Entrada de Usuario, pero se recibió un símbolo inválido`,
	);
	...
	if (upperCaseWords.includes(textCase))
		...
		modifiers.push((v: TextValue) =>
			makeText(v.value.slice(0, 1).toUpperCase() + v.value.slice(1).toLowerCase()),
		);
	else
		throw parser.TuberParserError(
			`Se esperaba: \`mayusculas\`, \`minusculas\`, \`capitales\` o similares en definición de directiva de Entrada de Usuario. Sin embargo, se recibió: \`${textCase}\``,
			caseToken,
		);
}

function parseReadStmtBooleanModifier(
	preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier[],
	parser: Parser,
) {
	...
	switch (name) {
		case 'como': {
			...
			modifiers.push((v: BooleanValue, it, scope) =>
				v.value ? it.evaluate(left, scope, false) : it.evaluate(right, scope, false),
			);
			break;
		}
			...
			preModifiers.push((v, it, scope) => {
				...
				if (it.is(truthy, ValueKinds.NADA))
					throw it.TuberInterpreterError(
						'El operando izquierdo en disyunción lógica de directiva de Entrada de Usuario Lógica "según" fue Nada',
					);

				if (it.is(falsy, ValueKinds.NADA))
					throw it.TuberInterpreterError(
						'El operando derecho en disyunción lógica de directiva de Entrada de Usuario Lógica "según" fue Nada',
					);
				...
				if (normTextValue === normTextTruthy) return 'Verdadero';

				if (normTextValue === normTextFalsy) return 'Falso';

				throw TuberInputError(
					`Se esperaba "${textTruthy}" o "${textFalsy}" para Entrada de Tipo Lógico. Sin embargo, se recibió: ${v}`,
				);
			});
			break;
		}
		...
		default:
			throw parser.TuberParserError(
				`Se esperaba una palabra clave existente en definición de directiva de Entrada de Usuario. Sin embargo, se recibió: \`${name}\``,
				token,
			);
	}
}
...
export function parseDeclarationStatement(parser: Parser): DeclarationStatement {
	...
	let lastIdentifier = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un identificador luego del tipo de dato (o solamente el identificador) en Sentencia \`CREAR\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
	...
	if (parser.current.is(TokenKinds.COMMA)) {
		while (parser.current.is(TokenKinds.COMMA)) {
			...
			if (!parser.hasTokens || parser.current.isStatement) break;

			lastIdentifier = parser.expect(
				TokenKinds.IDENTIFIER,
				`Se esperaban identificadores separados por \`,\` luego del tipo de dato en Sentencia \`CREAR\`. Sin embargo, se recibió: *${parser.current.translated}*`,
			);
...
export function parseSaveStatement(parser: Parser): SaveStatement {
	...
	const identifier = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un identificador en Sentencia \`GUARDAR\`, pero se recibió: *${parser.current.value}*`,
	);
	...
	if (parser.current.is(TokenKinds.ASSIGNMENT)) {
		parser.advance();
		parser.ensureExpression(
			`Se esperaba una expresión a guardar luego de \`con\` en Sentencia \`GUARDAR\`, pero se recibió: *${parser.current.value}*`,
		);
...
export function parseAssignmentStatement(parser: Parser): AssignmentStatement | LoadStatement {
	...
	if (
		receptor.kind !== ExpressionKinds.IDENTIFIER
		&& receptor.kind !== ExpressionKinds.ARROW
		&& receptor.kind !== ExpressionKinds.BINARY
	)
		throw parser.TuberParserError(
			`Se esperaba una expresión receptora asignable en Sentencia ${operatorString}. Sin embargo, se recibió: *${receptorToken.translated}*`,
			receptorToken,
		);
...
function parseSingleSideAssignment(
	parser: Parser,
	operator: Token,
	receptor: Expression,
): AssignmentStatement | LoadStatement | null {
	...
	if (receptor.left.kind !== ExpressionKinds.BINARY) return null;
	...
	function walkExpressions(expr: Expression): Expression[] {
		if (expr.kind === ExpressionKinds.BINARY) {
			if (!expr.operator.is(TokenKinds.OR) || expr.right.kind === ExpressionKinds.BINARY)
				throw parser.TuberParserError(
					'Solo se permiten expresiones binarias lógicas disyuntivas (`o`) sin agrupamientos en sentencias de Asignación Condicional',
				);
	...
	if (expressions[0].kind !== ExpressionKinds.IDENTIFIER)
		throw parser.TuberParserError(
			'La primera expresión de una Sentencia de Asignación Condicional debe ser un identificador',
		);
...
export function parseExtendStatement(parser: Parser): AssignmentStatement | InsertionStatement {
	...
	const parseReception = () => {
		parser.expect(
			TokenKinds.ASSIGNMENT,
			`Se esperaba \`con\` y una expresión luego de expresión receptora en Sentencia ${operatorString}. Sin embargo, se recibió: ${parser.current.translated}`,
		);
...
export function parseDeleteStatement(parser: Parser): DeleteStatement {
	...
	const identifier = parser.expect(
		TokenKinds.IDENTIFIER,
		`Se esperaba un identificador en Sentencia \`BORRAR\`, pero se recibió: *${parser.current.value}*`,
	);
...
export function parseReturnStatement(parser: Parser): ReturnStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión a devolver inmediatamente después de indicador \`${`${startToken.value}`.toUpperCase()}\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
	const expression = parser.parseExpression(BindingPowers.COMMA);
...
export function parseStopStatement(parser: Parser): StopStatement {
	...
	parser.expect(
		TokenKinds.ASSIGNMENT,
		`Se esperaba \`con\` en Sentencia \`PARAR\`, pero se recibió: *${parser.current.translated}*`,
	);

	parser.ensureExpression(
		`Se esperaba un mensaje de corte luego de \`con\` en Sentencia \`PARAR\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
	...
	parser.expect(
		TokenKinds.IF,
		`Se esperaba \`SI\` y una condición luego de mensaje de corte en Sentencia \`PARAR\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);

	parser.ensureExpression(
		`Se esperaba una expresión lógica luego de \`SI\` en Sentencia \`PARAR\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
...
export function parseSendStatement(parser: Parser): SendStatement {
	...
	parser.ensureExpression(
		`Se esperaba una expresión a enviar inmediatamente después de indicador \`${`${startToken.value}`.toUpperCase()}\`. Sin embargo, se recibió: *${parser.current.translated}*`,
	);
	const expression = parser.parseExpression(BindingPowers.COMMA);
