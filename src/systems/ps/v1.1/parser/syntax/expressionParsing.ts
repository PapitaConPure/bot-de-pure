	...
	switch (literal.kind) {
		...
		case TokenKinds.LIT_BOOLEAN:
			return {
				...
				value: toLowerCaseNormalized(literal.value as string) === 'verdadero',
				...metadata,
			};
		...
		case TokenKinds.LIST: {
			...
			const end = elements.length ? elements[elements.length - 1].end : literal.end;
		...
		default:
			throw new TypeError(
				`No se puede crear una expresión para: ${literal.translated ?? literal.kind}`,
			);
	}
}
...
export function canFollowUp(parser: Parser): boolean {
	return (
		...
		&& !parser.current.isAny(TokenKinds.PAREN_CLOSE, TokenKinds.OPTIONAL)
	);
}
	...
	switch (expr.kind) {
		...
		default:
			throw parser.TuberParserError(
				`Se esperaba un identificador, número, texto o lógico para analizar una clave de miembro de contenedor`,
			);
	}
...
export function parseRegistryMember(parser: Parser): RawRegistryMember {
	const key = parser
		.expectAny(TokenKinds.LIT_NUMBER, TokenKinds.LIT_TEXT, TokenKinds.IDENTIFIER)
		.orFail(
			`Se esperaba un identificador, una cadena o un número en lado izquierdo de expresión literal de miembro para expresión literal de glosario. Sin embargo, se recibió: ${parser.current.translated}`,
		);
...
export function parseConditionalExpression(
	...
	parser.expect(
		TokenKinds.COLON,
		`Se esperaba el operador \`:\` luego del operando medio en expresión ternaria. Sin embargo, se recibió: ${parser.current.translated}`,
	);
...
export function parseCallExpression(
	...
	if (!parser.current.is(TokenKinds.PAREN_CLOSE)) {
		...
		while (parser.hasTokens && parser.current.is(TokenKinds.COMMA)) {
			...
			if (parser.current.is(TokenKinds.COMMA))
				throw parser.TuberParserError(
					'No se pueden tener argumentos vacíos en una expresión de llamado',
				);

			//Si se recibe un indicador de sentencia después de la coma, se trata de una coma suelta, así que la ignoramos
			if (canFollowUp(parser)) args.push(parser.parseExpression(BindingPowers.COMMA));
		}
	}
...
export function parseArgument(parser: Parser, expression: Expression): ArgumentExpression {
	if (expression.kind !== ExpressionKinds.IDENTIFIER)
		throw parser.TuberParserError(
			'Se esperaba un identificador como argumento en expresión de Función',
		);
...
export function parseFunctionExpression(parser: Parser): FunctionExpression {
	...
	parser.expect(
		TokenKinds.PAREN_OPEN,
		`Se esperaba una apertura de paréntesis inmediatamente luego de indicación de expresión de Función, pero se recibió: ${parser.current.translated}`,
	);
	...
	if (!parser.current.is(TokenKinds.PAREN_CLOSE)) {
		...
		while (parser.hasTokens && parser.current.is(TokenKinds.COMMA)) {
			...
			if (parser.current.is(TokenKinds.COMMA))
				throw parser.TuberParserError(
					'No se pueden tener argumentos vacíos en una expresión de Función',
				);
	...
	parser.expect(
		...
		`Se esperaba un cierre de paréntesis luego de los parámetros de Función para proceder al cuerpo de la Función, pero se recibió: ${parser.current.translated}`,
	);

	const blockStart = parser.current;
	const blockBody = parseBlockBody(parser);
	const blockEnd = parser.expect(
...
export function parseSequenceExpression(
	...
	do {
		...
		if (!parser.hasTokens || parser.current.is(TokenKinds.COMMA))
			throw parser.TuberParserError(
				'Se esperaba una expresión después del operador de coma en expresión de secuencia',
			);

		//Ignorar comas sueltas
		if (canFollowUp(parser)) expressions.push(parser.parseExpression(BindingPowers.ASSIGNMENT));
	} while (parser.hasTokens && parser.current.is(TokenKinds.COMMA));
...
export function parseLambdaExpression(
	...
	if (left.kind !== ExpressionKinds.SEQUENCE && left.kind !== ExpressionKinds.IDENTIFIER)
		throw parser.TuberParserError(
			'Se esperaba una expresión de secuencia o un identificador antes del operador "tal que" en expresión lambda',
		);

	const args =
		left.kind === ExpressionKinds.SEQUENCE
