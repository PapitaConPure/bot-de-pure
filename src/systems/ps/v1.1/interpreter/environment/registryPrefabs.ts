...
import type { PSChannel, PSGuild, PSMember, PSRole } from './environmentProvider';
...
export function makeDiscordMember(member: PSMember): RegistryValue {
...
export function makeDiscordRole(role: PSRole): RegistryValue {
...
export function makeDiscordChannel(channel: PSChannel): RegistryValue {
...
export async function makeDiscordGuild(guild: PSGuild): Promise<RegistryValue> {
...
export function makeEmbedRegistry(embed: EmbedData): RegistryValue {
