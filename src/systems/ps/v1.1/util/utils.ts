...
export function improveNumber(
	...
	const googol = 10 ** 100;
	if (num >= googol) return `${formatNumber(num / googol, { maximumFractionDigits: 4 })} Gúgol`;
...
export function toLowerCaseNormalized(
	...
	text = text
		.replace(/[áéíóúàèìòùâêîôûäëïöüāēīōūãñçõ]/gi, (char) =>
			char.normalize('NFD').replace(/[\u0300-\u036f]/, ''),
		)
		.toLowerCase();

	if (removeCarriageReturns) text = text.replace(/\r/g, '');
...
export function rand(maxExclusive: number, round: boolean = true): number {
	...
	const value =
		((Date.now() * 0.001 + maxExclusive * Math.random()) % maxExclusive) * negativeHandler;
