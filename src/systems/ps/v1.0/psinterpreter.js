...
class TuberInterpreter {
	...
	#sendStack = [];
	/**@type {import('../../../src/commands/commons/typings.js').CommandRequest}*/
	#request;
	/**@type {Boolean}*/
	#testDrive;
	...
	#evaluateStatement(node, scope) {
		...
		case ParserNodeTypes.Conditional: {
			const assertedNode = /**@type {import('../../../src/types/util.js').Flatten<import('./commons.js').ParserConditionalStatementNode>}*/(node);
			const test = makeValue(this.#evaluate(assertedNode.test, scope), 'Boolean');
			const blockScope = new TuberScope(scope);
			if(test.value)
