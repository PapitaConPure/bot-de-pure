 ...
 */
export async function tryRecoverSavedTracksQueue(
	request: ComplexCommandRequest | Interaction,
	...
	const [currentTrack, ...restOfTracks] = savedQueue.serializedTracks.map((st) =>
		deserializeTrack(player, st),
	);
...
async function attemptDatabaseQueueRecovery(
	data: __attempDatabaseQueueRecoveryData,
	retries: number = 3,
) {
	const { request, pauseOnInit, currentTrack, restOfTracks } = data;

	const channel = (request.member as GuildMember).voice?.channel;
	if (!channel) return null;

	console.log(
		'Request member is in voice channel. Recovering "current track" and rest of queue...',
	);
	try {
		const player = useMainPlayer();
