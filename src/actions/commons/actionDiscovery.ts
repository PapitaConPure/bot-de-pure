...
export async function fetchActionsFromFiles(options: FetchActionOptions = {}): Promise<Action[]> {
	...
	const commandModules = await Promise.all(actionFilenames.map(async filename => ({
		filename,
		commandModule: await import(`../instances/${filename}`),
	})));

	for(const { filename, commandModule} of commandModules) {
		if(commandModule instanceof Action)
			pushOrDiscard(commandModule);
		else if(commandModule.default instanceof Action)
			pushOrDiscard(commandModule.default);
		else if(commandModule.command instanceof Action)
			pushOrDiscard(commandModule.command);
		else
			throw new FetchActionError(`No se encontró un comando en el módulo: ${filename} desde ${__dirname}`);
	}
